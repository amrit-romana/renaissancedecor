...
const SocialIcon = ({ name }: { name: string }): ReactNode => {
...
export function Footer() {
  ...
  useEffect(() => {
    if (!pathname?.startsWith("/admin")) {
      getDbData().then(db => setFooterData(db.footerSettings || {}));
    }
  }, [pathname]);
  ...
  const f = footerData;
  if (!f) return null; // Wait until data loads
  ...
  return (
    <footer className="bg-[#000000] text-[var(--color-parchment)] py-24 px-8 md:px-16 w-full flex flex-col mt-auto z-10 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16 w-full mb-24 items-center">
        ...
        <div className="flex flex-col gap-8 font-sans w-full">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm md:text-base leading-relaxed" style={{ whiteSpace: "pre-line" }}>
              {f.address || "Unit 5 / 314 Governor Road,\nBraeside 3195"}
            </p>
            <p className="text-xs md:text-sm text-[var(--color-parchment)]/80 mt-1 italic">
              {f.addressNote || "Visits by appointment only"}
            </p>
          </div>

          <div className="flex flex-col gap-1">
            <p className="font-bold text-[10px] md:text-xs tracking-widest uppercase text-[var(--color-parchment)]/70 mb-1">Talk to Us</p>
            <a href={`tel:${f.phone?.replace(/\s/g,"")}`} className="text-sm md:text-base hover:opacity-60 transition-opacity">{f.phone || "0468 326 303"}</a>
            <p className="text-xs md:text-sm text-[var(--color-parchment)]/80 mt-1">{f.openingHours || "9:00am — 5:00pm, Mon – Fri"}</p>
          </div>

          <div className="mt-2">
            <p className="text-[10px] md:text-xs tracking-[0.15em] text-[var(--color-parchment)]/40 uppercase">
              Renaissance Decor | {f.abn || "ABN 70 890 172 250"}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 font-sans w-full">
          {director.name && (
            <div className="flex flex-col gap-1 border-b border-[var(--color-parchment)]/30 pb-3">
              <p className="font-bold text-[10px] md:text-xs tracking-widest uppercase text-[var(--color-parchment)]/70">{director.name}</p>
              {director.email && <a href={`mailto:${director.email}`} className="text-sm md:text-base hover:opacity-60 transition-opacity mt-1">{director.email}</a>}
            </div>
          )}
          {marketing.name && (
            <div className="flex flex-col gap-1 border-b border-[var(--color-parchment)]/30 pb-3">
              <p className="font-bold text-[10px] md:text-xs tracking-widest uppercase text-[var(--color-parchment)]/70">{marketing.name}</p>
              {marketing.email && <a href={`mailto:${marketing.email}`} className="text-sm md:text-base hover:opacity-60 transition-opacity mt-1">{marketing.email}</a>}
            </div>
          )}
          <div className="flex flex-col gap-3 mt-2">
            ...
            <div className="flex flex-row gap-4 mt-2">
              {Object.entries(socials).map(([name, url]) =>
                url && url !== "#" ? (
                  <a key={name} href={url as string} target="_blank" rel="noopener noreferrer" className="hover:opacity-60 transition-opacity" title={name.charAt(0).toUpperCase() + name.slice(1)}>
                    <SocialIcon name={name} />
                  </a>
                ) : null
              )}
              {/* Fallback static icons if no socials set */}
              {Object.values(socials).every(v => !v || v === "#") && (
                ["instagram","facebook","pinterest","tiktok","linkedin"].map(name => (
                  <a key={name} href="#" className="hover:opacity-60 transition-opacity" title={name}>
                    <SocialIcon name={name} />
                  </a>
                ))
              )}
      ...
      <div className="w-full flex flex-col md:flex-row justify-between items-center text-[10px] md:text-xs font-sans tracking-widest text-[var(--color-parchment)]/60 pt-8 border-t border-[var(--color-parchment)]/20">
        <p>All content copyright © {new Date().getFullYear()} {f.copyrightName || "Renaissance Decor"}. All rights reserved.</p>
